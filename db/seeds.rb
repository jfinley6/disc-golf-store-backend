...
def populate_discs
    ...
    array.each do |s|
        Disc.create(name: s["name"], brand: s["brand"], category: s["category"], speed: s["speed"].to_i, glide: s["glide"].to_i, turn: s["turn"].to_i, fade: s["fade"].to_i, stability: s["stability"], flight_path: s["pic"], category_slug: s["category_slug"], price: prices.sample, picture_url: pictures.sample, name_slug: s["name_slug"])
    end
end
